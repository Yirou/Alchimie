    ...
    models.E_role.findOne({
        ...
        e_role['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function(count) {
            ...
            for (var i = 0; i < e_role[subentityAlias].length; i++)
                rawData.data.push(e_role[subentityAlias][i].get({
                    plain: true
                }));
    ...
    models.E_role.create(createObject).then(function(e_role) {
        ...
        model_builder.setAssocationManyValues(e_role, req.body, createObject, options).then(function() {
            Promise.all(promises).then(function() {
                ...
                entity_helper.error(err, req, res, '/role/create_form');
            });
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/role/create_form');
...
router.get('/update_form', block_access.actionAccessMiddleware("role", "update"), function(req, res) {
    ...
    entity_helper.optimizedFindOne('E_role', id_e_role, options).then(function(e_role) {
        ...
        entity_helper.getPicturesBuffers(e_role, "e_role", true).then(function() {
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(req.query.ajax ? e_role.dataValues : data, options).then(function(data) {
                if (req.query.ajax) {
    ...
    models.E_role.findOne({
        ...
        e_role.update(updateObject).then(function() {
            ...
            model_builder.setAssocationManyValues(e_role, req.body, updateObject, options).then(function() {
                ...
                entity_helper.error(err, req, res, '/role/update_form?id=' + id_e_role);
            });
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/role/update_form?id=' + id_e_role);
        });
...
router.get('/set_status/:id_role/:status/:id_new_status', block_access.actionAccessMiddleware("role", "read"), function(req, res) {
    status_helper.setStatus('e_role', req.params.id_role, req.params.status, req.params.id_new_status, req.query.comment).then(()=> {
        res.redirect(req.headers.referer);
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/role/show?id=' + req.params.id_role);
    });
});
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("role", "delete"), function(req, res) {
    ...
    models.E_role.findOne({
        ...
        e_role['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(function(aliasEntities) {
...
router.post('/delete', block_access.actionAccessMiddleware("role", "delete"), function(req, res) {
    ...
    models.E_role.findOne({
        ...
        models.E_role.destroy({
            ...
            entity_helper.error(err, req, res, '/role/list');
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/role/list');
    });
