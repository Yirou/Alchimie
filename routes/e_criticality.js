...
var attributes = require('../models/attributes/e_criticality');
var options = require('../models/options/e_criticality');
...
router.get('/list', block_access.actionAccessMiddleware("criticality", "read"), function(req, res) {
    res.render('e_criticality/list');
});

router.post('/datalist', block_access.actionAccessMiddleware("criticality", "read"), function(req, res) {
    /* Looking for include to get all associated related to data for the datalist ajax loading */
    var include = model_builder.getDatalistInclude(models, options, req.body.columns);
    filterDataTable("E_criticality", req.body, include).then(function(rawData) {
        entity_helper.prepareDatalistResult('e_criticality', rawData, req.session.lang_user).then(function(preparedData) {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("criticality", "read"), function(req, res) {
    ...
    models.E_criticality.findOne({
        where: {
            id: parseInt(sourceId)
        },
        include: include
    }).then(function(e_criticality) {
        if (!e_criticality['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]) {
            console.error('/subdatalist: count' + entity_helper.capitalizeFirstLetter(subentityAlias) + ' is undefined');
            return res.status(500).end();
        }

        e_criticality['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function(count) {
            var rawData = {
                recordsTotal: count,
                recordsFiltered: count,
                data: []
            };
            for (var i = 0; i < e_criticality[subentityAlias].length; i++)
                rawData.data.push(e_criticality[subentityAlias][i].get({
                    plain: true
                }));
...
router.get('/show', block_access.actionAccessMiddleware("criticality", "read"), function(req, res) {
    var id_e_criticality = req.query.id;
    var tab = req.query.tab;
    var data = {
        tab: tab,
        enum_radio: enums_radios.translated("e_criticality", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_criticality', id_e_criticality, options).then(function(e_criticality) {
        if (!e_criticality) {
            data.error = 404;
        ...
        /* Update local e_criticality data before show */
        data.e_criticality = e_criticality;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_criticality, "e_criticality").then(function() {
            status_helper.translate(e_criticality, attributes, req.session.lang_user);
            data.componentAddressConfig = component_helper.getMapsConfigIfComponentAddressExist("e_criticality");
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(data, options).then(function(data) {
                res.render('e_criticality/show', data);
            }).catch(function(err) {
...
router.get('/create_form', block_access.actionAccessMiddleware("criticality", "create"), function(req, res) {
    var data = {
        enum_radio: enums_radios.translated("e_criticality", req.session.lang_user, options)
    };
    ...
    // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
    entity_helper.getLoadOnStartData(data, options).then(function(data) {
        var view = req.query.ajax ? 'e_criticality/create_fields' : 'e_criticality/create';
...
router.post('/create', block_access.actionAccessMiddleware("criticality", "create"), function(req, res) {

    var createObject = model_builder.buildForRoute(attributes, options, req.body);

    models.E_criticality.create(createObject).then(function(e_criticality) {
        var redirect = '/criticality/show?id=' + e_criticality.id;
        req.session.toastr = [{
            ...
            promises.push(new Promise(function(resolve, reject) {
                models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({
                    ...
                    if (typeof association['add' + modelName] !== 'undefined') {
                        association['add' + modelName](e_criticality.id).then(resolve).catch(function(err) {
                            reject(err);
                        });
                    } else {
                        var obj = {};
                        obj[req.body.associationForeignKey] = e_criticality.id;
        ...
        model_builder.setAssocationManyValues(e_criticality, req.body, createObject, options).then(function() {
            Promise.all(promises).then(function() {
                component_helper.setAddressIfComponentExist(e_criticality, options, req.body).then(function() {
                    res.redirect(redirect);
                });
            }).catch(function(err) {
                entity_helper.error(err, req, res, '/criticality/create_form');
            });
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/criticality/create_form');
...
router.get('/update_form', block_access.actionAccessMiddleware("criticality", "update"), function(req, res) {
    var id_e_criticality = req.query.id;
    var data = {
        enum_radio: enums_radios.translated("e_criticality", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_criticality', id_e_criticality, options).then(function(e_criticality) {
        ...
        e_criticality.dataValues.enum_radio = data.enum_radio;
        data.e_criticality = e_criticality;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_criticality, "e_criticality", true).then(function() {
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(req.query.ajax ? e_criticality.dataValues : data, options).then(function(data) {
                if (req.query.ajax) {
                    e_criticality.dataValues = data;
                    res.render('e_criticality/update_fields', e_criticality.get({
                        plain: true
                    }));
                } else
                    res.render('e_criticality/update', data);
            }).catch(function(err) {
                entity_helper.error(err, req, res, "/");
            })
        }).catch(function(err) {
...
router.post('/update', block_access.actionAccessMiddleware("criticality", "update"), function(req, res) {
    var id_e_criticality = parseInt(req.body.id);
    ...
    models.E_criticality.findOne({
        ...
        component_helper.updateAddressIfComponentExist(e_criticality, options, req.body);
        e_criticality.update(updateObject).then(function() {

            // We have to find value in req.body that are linked to an hasMany or belongsToMany association
            // because those values are not updated for now
            model_builder.setAssocationManyValues(e_criticality, req.body, updateObject, options).then(function() {

                var redirect = '/criticality/show?id=' + id_e_criticality;
                if (typeof req.body.associationFlag !== 'undefined')
                    redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
                ...
                entity_helper.error(err, req, res, '/criticality/update_form?id=' + id_e_criticality);
            });
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/criticality/update_form?id=' + id_e_criticality);
        });
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('criticality', 'read'), function(req, res) {
    ...
    // Fetch tab data
    models.E_criticality.findOne(queryOpts).then(function(e_criticality) {
        if (!e_criticality)
            return res.status(404).end();

        var dustData = e_criticality[option.as] || null;
...
router.get('/set_status/:id_criticality/:status/:id_new_status', block_access.actionAccessMiddleware("criticality", "read"), function(req, res) {
    status_helper.setStatus('e_criticality', req.params.id_criticality, req.params.status, req.params.id_new_status, req.query.comment).then(()=> {
        res.redirect(req.headers.referer);
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/criticality/show?id=' + req.params.id_criticality);
    });
});

router.post('/search', block_access.actionAccessMiddleware('criticality', 'read'), function(req, res) {
    ...
    models.E_criticality.findAndCountAll(where).then(function(results) {
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("criticality", "delete"), function(req, res) {
    var alias = req.params.alias;
    ...
    models.E_criticality.findOne({
        ...
        e_criticality['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(function(aliasEntities) {
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("criticality", "create"), function(req, res) {
    var alias = req.params.alias;
    var idEntity = req.body.idEntity;
    models.E_criticality.findOne({
        where: {
        ...
        if (typeof(toAdd = req.body.ids) === 'undefined') {
            ...
            return res.redirect('/criticality/show?id=' + idEntity + "#" + alias);
        }

        e_criticality['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(function() {
            res.redirect('/criticality/show?id=' + idEntity + "#" + alias);
        }).catch(function(err) {
            entity_helper.error(err, req, res, "/");
        });
    }).catch(function(err) {
...
router.post('/delete', block_access.actionAccessMiddleware("criticality", "delete"), function(req, res) {
    var id_e_criticality = parseInt(req.body.id);
    ...
    models.E_criticality.findOne({
        ...
        models.E_criticality.destroy({
            ...
            var redirect = '/criticality/list';
            if (typeof req.body.associationFlag !== 'undefined')
                redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
            res.redirect(redirect);
            entity_helper.remove_files("e_criticality", deleteObject, attributes);
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/criticality/list');
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/criticality/list');
    });
