...
var attributes = require('../models/attributes/e_application_alert');
var options = require('../models/options/e_application_alert');
...
router.get('/list', block_access.actionAccessMiddleware("application_alert", "read"), function(req, res) {
    res.render('e_application_alert/list');
});

router.post('/datalist', block_access.actionAccessMiddleware("application_alert", "read"), function(req, res) {
    /* Looking for include to get all associated related to data for the datalist ajax loading */
    var include = model_builder.getDatalistInclude(models, options, req.body.columns);
    filterDataTable("E_application_alert", req.body, include).then(function(rawData) {
        entity_helper.prepareDatalistResult('e_application_alert', rawData, req.session.lang_user).then(function(preparedData) {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("application_alert", "read"), function(req, res) {
    ...
    models.E_application_alert.findOne({
        ...
        if (!e_application_alert['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]) {
            console.error('/subdatalist: count' + entity_helper.capitalizeFirstLetter(subentityAlias) + ' is undefined');
            return res.status(500).end();
        }

        e_application_alert['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function(count) {
            var rawData = {
                recordsTotal: count,
                recordsFiltered: count,
                data: []
            };
            for (var i = 0; i < e_application_alert[subentityAlias].length; i++)
                rawData.data.push(e_application_alert[subentityAlias][i].get({
                    plain: true
                }));
...
router.get('/show', block_access.actionAccessMiddleware("application_alert", "read"), function(req, res) {
    var id_e_application_alert = req.query.id;
    var tab = req.query.tab;
    var data = {
        tab: tab,
        enum_radio: enums_radios.translated("e_application_alert", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_application_alert', id_e_application_alert, options).then(function(e_application_alert) {
        ...
        /* Update local e_application_alert data before show */
        data.e_application_alert = e_application_alert;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_application_alert, "e_application_alert").then(function() {
            status_helper.translate(e_application_alert, attributes, req.session.lang_user);
            data.componentAddressConfig = component_helper.getMapsConfigIfComponentAddressExist("e_application_alert");
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(data, options).then(function(data) {
                res.render('e_application_alert/show', data);
            }).catch(function(err) {
...
router.get('/create_form', block_access.actionAccessMiddleware("application_alert", "create"), function(req, res) {
    var data = {
        enum_radio: enums_radios.translated("e_application_alert", req.session.lang_user, options)
    };
    ...
    // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
    entity_helper.getLoadOnStartData(data, options).then(function(data) {
        var view = req.query.ajax ? 'e_application_alert/create_fields' : 'e_application_alert/create';
...
router.post('/create', block_access.actionAccessMiddleware("application_alert", "create"), function(req, res) {

    var createObject = model_builder.buildForRoute(attributes, options, req.body);

    models.E_application_alert.create(createObject).then(function(e_application_alert) {
        var redirect = '/application_alert/show?id=' + e_application_alert.id;
        req.session.toastr = [{
            ...
            promises.push(new Promise(function(resolve, reject) {
                models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({
                    ...
                    if (typeof association['add' + modelName] !== 'undefined') {
                        association['add' + modelName](e_application_alert.id).then(resolve).catch(function(err) {
                            reject(err);
                        });
                    } else {
                        var obj = {};
                        obj[req.body.associationForeignKey] = e_application_alert.id;
        ...
        model_builder.setAssocationManyValues(e_application_alert, req.body, createObject, options).then(function() {
            Promise.all(promises).then(function() {
                component_helper.setAddressIfComponentExist(e_application_alert, options, req.body).then(function() {
                    res.redirect(redirect);
                });
            }).catch(function(err) {
                entity_helper.error(err, req, res, '/application_alert/create_form');
            });
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/application_alert/create_form');
...
router.get('/update_form', block_access.actionAccessMiddleware("application_alert", "update"), function(req, res) {
    var id_e_application_alert = req.query.id;
    var data = {
        enum_radio: enums_radios.translated("e_application_alert", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_application_alert', id_e_application_alert, options).then(function(e_application_alert) {
        ...
        e_application_alert.dataValues.enum_radio = data.enum_radio;
        data.e_application_alert = e_application_alert;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_application_alert, "e_application_alert", true).then(function() {
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(req.query.ajax ? e_application_alert.dataValues : data, options).then(function(data) {
                if (req.query.ajax) {
                    e_application_alert.dataValues = data;
                    res.render('e_application_alert/update_fields', e_application_alert.get({
                        plain: true
                    }));
                } else
                    res.render('e_application_alert/update', data);
            }).catch(function(err) {
                entity_helper.error(err, req, res, "/");
            })
        }).catch(function(err) {
...
router.post('/update', block_access.actionAccessMiddleware("application_alert", "update"), function(req, res) {
    var id_e_application_alert = parseInt(req.body.id);
    ...
    models.E_application_alert.findOne({
        ...
        component_helper.updateAddressIfComponentExist(e_application_alert, options, req.body);
        e_application_alert.update(updateObject).then(function() {

            // We have to find value in req.body that are linked to an hasMany or belongsToMany association
            // because those values are not updated for now
            model_builder.setAssocationManyValues(e_application_alert, req.body, updateObject, options).then(function() {

                var redirect = '/application_alert/show?id=' + id_e_application_alert;
                if (typeof req.body.associationFlag !== 'undefined')
                    redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
                ...
                entity_helper.error(err, req, res, '/application_alert/update_form?id=' + id_e_application_alert);
            });
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/application_alert/update_form?id=' + id_e_application_alert);
        });
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('application_alert', 'read'), function(req, res) {
    ...
    // Fetch tab data
    models.E_application_alert.findOne(queryOpts).then(function(e_application_alert) {
        if (!e_application_alert)
            return res.status(404).end();

        var dustData = e_application_alert[option.as] || null;
...
router.get('/set_status/:id_application_alert/:status/:id_new_status', block_access.actionAccessMiddleware("application_alert", "read"), function(req, res) {
    status_helper.setStatus('e_application_alert', req.params.id_application_alert, req.params.status, req.params.id_new_status, req.query.comment).then(()=> {
        res.redirect(req.headers.referer);
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/application_alert/show?id=' + req.params.id_application_alert);
    });
});

router.post('/search', block_access.actionAccessMiddleware('application_alert', 'read'), function(req, res) {
    ...
    models.E_application_alert.findAndCountAll(where).then(function(results) {
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("application_alert", "delete"), function(req, res) {
    var alias = req.params.alias;
    ...
    models.E_application_alert.findOne({
        ...
        e_application_alert['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(function(aliasEntities) {
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("application_alert", "create"), function(req, res) {
    var alias = req.params.alias;
    var idEntity = req.body.idEntity;
    models.E_application_alert.findOne({
        where: {
        ...
        if (typeof(toAdd = req.body.ids) === 'undefined') {
            ...
            return res.redirect('/application_alert/show?id=' + idEntity + "#" + alias);
        }

        e_application_alert['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(function() {
            res.redirect('/application_alert/show?id=' + idEntity + "#" + alias);
        }).catch(function(err) {
            entity_helper.error(err, req, res, "/");
        });
    }).catch(function(err) {
...
router.post('/delete', block_access.actionAccessMiddleware("application_alert", "delete"), function(req, res) {
    var id_e_application_alert = parseInt(req.body.id);
    ...
    models.E_application_alert.findOne({
        ...
        models.E_application_alert.destroy({
            where: {
                id: id_e_application_alert
            }
        }).then(function() {
            ...
            var redirect = '/application_alert/list';
            if (typeof req.body.associationFlag !== 'undefined')
                redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
            res.redirect(redirect);
            entity_helper.remove_files("e_application_alert", deleteObject, attributes);
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/application_alert/list');
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/application_alert/list');
    });
