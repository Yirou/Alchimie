...
router.get('/list', block_access.actionAccessMiddleware("organization", "read"), function(req, res) {
    res.render('e_organization/list');
});
    ...
    models.E_organization.findOne({
        ...
        e_organization['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function(count) {
            ...
            for (var i = 0; i < e_organization[subentityAlias].length; i++)
                rawData.data.push(e_organization[subentityAlias][i].get({
                    plain: true
                }));
...
router.get('/show', block_access.actionAccessMiddleware("organization", "read"), function(req, res) {
    ...
    entity_helper.optimizedFindOne('E_organization', id_e_organization, options).then(function(e_organization) {
        ...
        entity_helper.getPicturesBuffers(e_organization, "e_organization").then(function() {
            status_helper.translate(e_organization, attributes, req.session.lang_user);
    ...
    models.E_organization.create(createObject).then(function(e_organization) {
        ...
        model_builder.setAssocationManyValues(e_organization, req.body, createObject, options).then(function() {
            Promise.all(promises).then(function() {
                ...
                entity_helper.error(err, req, res, '/organization/create_form');
            });
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/organization/create_form');
...
router.get('/update_form', block_access.actionAccessMiddleware("organization", "update"), function(req, res) {
    ...
    entity_helper.optimizedFindOne('E_organization', id_e_organization, options).then(function(e_organization) {
        ...
        entity_helper.getPicturesBuffers(e_organization, "e_organization", true).then(function() {
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(req.query.ajax ? e_organization.dataValues : data, options).then(function(data) {
                if (req.query.ajax) {
                    e_organization.dataValues = data;
                    res.render('e_organization/update_fields', e_organization.get({
                        plain: true
                    }));
                } else
    ...
    models.E_organization.findOne({
        ...
        e_organization.update(updateObject).then(function() {
            ...
            model_builder.setAssocationManyValues(e_organization, req.body, updateObject, options).then(function() {
                ...
                entity_helper.error(err, req, res, '/organization/update_form?id=' + id_e_organization);
            });
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/organization/update_form?id=' + id_e_organization);
        });
...
router.get('/set_status/:id_organization/:status/:id_new_status', block_access.actionAccessMiddleware("organization", "read"), function(req, res) {
    status_helper.setStatus('e_organization', req.params.id_organization, req.params.status, req.params.id_new_status, req.query.comment).then(()=> {
        res.redirect(req.headers.referer);
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/organization/show?id=' + req.params.id_organization);
    });
});
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("organization", "delete"), function(req, res) {
    ...
    models.E_organization.findOne({
        ...
        e_organization['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(function(aliasEntities) {
...
router.post('/delete', block_access.actionAccessMiddleware("organization", "delete"), function(req, res) {
    ...
    models.E_organization.findOne({
        ...
        models.E_organization.destroy({
            ...
            entity_helper.error(err, req, res, '/organization/list');
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/organization/list');
    });
