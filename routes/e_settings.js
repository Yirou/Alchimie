...
router.get('/list', block_access.actionAccessMiddleware("settings", "read"), function(req, res) {
    res.render('e_settings/list');
});

router.post('/datalist', block_access.actionAccessMiddleware("settings", "read"), function(req, res) {
    /* Looking for include to get all associated related to data for the datalist ajax loading */
    var include = model_builder.getDatalistInclude(models, options, req.body.columns);
    filterDataTable("E_settings", req.body, include).then(function(rawData) {
        entity_helper.prepareDatalistResult('e_settings', rawData, req.session.lang_user).then(function(preparedData) {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("settings", "read"), function(req, res) {
    ...
    models.E_settings.findOne({
        ...
        if (!e_settings['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]) {
            console.error('/subdatalist: count' + entity_helper.capitalizeFirstLetter(subentityAlias) + ' is undefined');
            return res.status(500).end();
        }

        e_settings['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function(count) {
            var rawData = {
                recordsTotal: count,
                recordsFiltered: count,
                data: []
            };
            for (var i = 0; i < e_settings[subentityAlias].length; i++)
                rawData.data.push(e_settings[subentityAlias][i].get({
                    plain: true
                }));
...
router.get('/show', block_access.actionAccessMiddleware("settings", "read"), function(req, res) {
    var id_e_settings = req.query.id;
    var tab = req.query.tab;
    var data = {
        tab: tab,
        enum_radio: enums_radios.translated("e_settings", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_settings', id_e_settings, options).then(function(e_settings) {
        if (!e_settings) {
            data.error = 404;
        ...
        /* Update local e_settings data before show */
        data.e_settings = e_settings;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_settings, "e_settings").then(function() {
            status_helper.translate(e_settings, attributes, req.session.lang_user);
            data.componentAddressConfig = component_helper.getMapsConfigIfComponentAddressExist("e_settings");
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(data, options).then(function(data) {
                res.render('e_settings/show', data);
            }).catch(function(err) {
...
router.get('/create_form', block_access.actionAccessMiddleware("settings", "create"), function(req, res) {
    var data = {
        enum_radio: enums_radios.translated("e_settings", req.session.lang_user, options)
    };
    ...
    // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
    entity_helper.getLoadOnStartData(data, options).then(function(data) {
        var view = req.query.ajax ? 'e_settings/create_fields' : 'e_settings/create';
...
router.post('/create', block_access.actionAccessMiddleware("settings", "create"), function(req, res) {

    var createObject = model_builder.buildForRoute(attributes, options, req.body);

    models.E_settings.create(createObject).then(function(e_settings) {
        var redirect = '/settings/show?id=' + e_settings.id;
        req.session.toastr = [{
            ...
            promises.push(new Promise(function(resolve, reject) {
                models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({
                    ...
                    if (typeof association['add' + modelName] !== 'undefined') {
                        association['add' + modelName](e_settings.id).then(resolve).catch(function(err) {
                            reject(err);
                        });
                    } else {
                        var obj = {};
                        obj[req.body.associationForeignKey] = e_settings.id;
        ...
        model_builder.setAssocationManyValues(e_settings, req.body, createObject, options).then(function() {
            Promise.all(promises).then(function() {
                component_helper.setAddressIfComponentExist(e_settings, options, req.body).then(function() {
                    res.redirect(redirect);
                });
            }).catch(function(err) {
                entity_helper.error(err, req, res, '/settings/create_form');
            });
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/settings/create_form');
...
router.get('/update_form', block_access.actionAccessMiddleware("settings", "update"), function(req, res) {
    var id_e_settings = req.query.id;
    var data = {
        enum_radio: enums_radios.translated("e_settings", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_settings', id_e_settings, options).then(function(e_settings) {
        ...
        e_settings.dataValues.enum_radio = data.enum_radio;
        data.e_settings = e_settings;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_settings, "e_settings", true).then(function() {
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(req.query.ajax ? e_settings.dataValues : data, options).then(function(data) {
                if (req.query.ajax) {
                    e_settings.dataValues = data;
                    res.render('e_settings/update_fields', e_settings.get({
                        plain: true
                    }));
                } else
                    res.render('e_settings/update', data);
            }).catch(function(err) {
                entity_helper.error(err, req, res, "/");
            })
        }).catch(function(err) {
...
router.post('/update', block_access.actionAccessMiddleware("settings", "update"), function(req, res) {
    var id_e_settings = parseInt(req.body.id);
    ...
    models.E_settings.findOne({
        ...
        component_helper.updateAddressIfComponentExist(e_settings, options, req.body);
        e_settings.update(updateObject).then(function() {

            // We have to find value in req.body that are linked to an hasMany or belongsToMany association
            // because those values are not updated for now
            model_builder.setAssocationManyValues(e_settings, req.body, updateObject, options).then(function() {

                var redirect = '/settings/show?id=' + id_e_settings;
                if (typeof req.body.associationFlag !== 'undefined')
                    redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
                ...
                entity_helper.error(err, req, res, '/settings/update_form?id=' + id_e_settings);
            });
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/settings/update_form?id=' + id_e_settings);
        });
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('settings', 'read'), function(req, res) {
    ...
    // Fetch tab data
    models.E_settings.findOne(queryOpts).then(function(e_settings) {
        if (!e_settings)
            return res.status(404).end();

        var dustData = e_settings[option.as] || null;
...
router.get('/set_status/:id_settings/:status/:id_new_status', block_access.actionAccessMiddleware("settings", "read"), function(req, res) {
    status_helper.setStatus('e_settings', req.params.id_settings, req.params.status, req.params.id_new_status, req.query.comment).then(()=> {
        res.redirect(req.headers.referer);
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/settings/show?id=' + req.params.id_settings);
    });
});

router.post('/search', block_access.actionAccessMiddleware('settings', 'read'), function(req, res) {
    ...
    models.E_settings.findAndCountAll(where).then(function(results) {
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("settings", "delete"), function(req, res) {
    var alias = req.params.alias;
    ...
    models.E_settings.findOne({
        ...
        e_settings['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(function(aliasEntities) {
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("settings", "create"), function(req, res) {
    var alias = req.params.alias;
    var idEntity = req.body.idEntity;
    models.E_settings.findOne({
        where: {
        ...
        if (typeof(toAdd = req.body.ids) === 'undefined') {
            ...
            return res.redirect('/settings/show?id=' + idEntity + "#" + alias);
        }

        e_settings['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(function() {
            res.redirect('/settings/show?id=' + idEntity + "#" + alias);
        }).catch(function(err) {
            entity_helper.error(err, req, res, "/");
        });
    }).catch(function(err) {
...
router.post('/delete', block_access.actionAccessMiddleware("settings", "delete"), function(req, res) {
    var id_e_settings = parseInt(req.body.id);
    ...
    models.E_settings.findOne({
        ...
        models.E_settings.destroy({
            ...
            var redirect = '/settings/list';
            if (typeof req.body.associationFlag !== 'undefined')
                redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
            res.redirect(redirect);
            entity_helper.remove_files("e_settings", deleteObject, attributes);
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/settings/list');
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/settings/list');
    });
