...
router.get('/list', block_access.actionAccessMiddleware("server_status_history", "read"), function(req, res) {
    res.render('e_server_status_history/list');
});

router.post('/datalist', block_access.actionAccessMiddleware("server_status_history", "read"), function(req, res) {
    /* Looking for include to get all associated related to data for the datalist ajax loading */
    var include = model_builder.getDatalistInclude(models, options, req.body.columns);
    filterDataTable("E_server_status_history", req.body, include).then(function(rawData) {
        entity_helper.prepareDatalistResult('e_server_status_history', rawData, req.session.lang_user).then(function(preparedData) {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("server_status_history", "read"), function(req, res) {
    ...
    models.E_server_status_history.findOne({
        ...
        if (!e_server_status_history['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]) {
            console.error('/subdatalist: count' + entity_helper.capitalizeFirstLetter(subentityAlias) + ' is undefined');
            return res.status(500).end();
        }

        e_server_status_history['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function(count) {
            var rawData = {
                recordsTotal: count,
                recordsFiltered: count,
                data: []
            };
            for (var i = 0; i < e_server_status_history[subentityAlias].length; i++)
                rawData.data.push(e_server_status_history[subentityAlias][i].get({
                    plain: true
                }));
...
router.get('/show', block_access.actionAccessMiddleware("server_status_history", "read"), function(req, res) {
    var id_e_server_status_history = req.query.id;
    var tab = req.query.tab;
    var data = {
        tab: tab,
        enum_radio: enums_radios.translated("e_server_status_history", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_server_status_history', id_e_server_status_history, options).then(function(e_server_status_history) {
        ...
        /* Update local e_server_status_history data before show */
        data.e_server_status_history = e_server_status_history;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_server_status_history, "e_server_status_history").then(function() {
            status_helper.translate(e_server_status_history, attributes, req.session.lang_user);
            data.componentAddressConfig = component_helper.getMapsConfigIfComponentAddressExist("e_server_status_history");
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(data, options).then(function(data) {
                res.render('e_server_status_history/show', data);
            }).catch(function(err) {
...
router.get('/create_form', block_access.actionAccessMiddleware("server_status_history", "create"), function(req, res) {
    var data = {
        enum_radio: enums_radios.translated("e_server_status_history", req.session.lang_user, options)
    };
    ...
    // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
    entity_helper.getLoadOnStartData(data, options).then(function(data) {
        var view = req.query.ajax ? 'e_server_status_history/create_fields' : 'e_server_status_history/create';
...
router.post('/create', block_access.actionAccessMiddleware("server_status_history", "create"), function(req, res) {

    var createObject = model_builder.buildForRoute(attributes, options, req.body);

    models.E_server_status_history.create(createObject).then(function(e_server_status_history) {
        var redirect = '/server_status_history/show?id=' + e_server_status_history.id;
        req.session.toastr = [{
        ...
        if (typeof req.body.associationFlag !== 'undefined') {
            ...
            promises.push(new Promise(function(resolve, reject) {
                models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({
                    ...
                    if (!association) {
                        e_server_status_history.destroy();
                        var err = new Error();
                    ...
                    if (typeof association['add' + modelName] !== 'undefined') {
                        association['add' + modelName](e_server_status_history.id).then(resolve).catch(function(err) {
                            reject(err);
                        });
                    } else {
                        var obj = {};
                        obj[req.body.associationForeignKey] = e_server_status_history.id;
        ...
        model_builder.setAssocationManyValues(e_server_status_history, req.body, createObject, options).then(function() {
            Promise.all(promises).then(function() {
                component_helper.setAddressIfComponentExist(e_server_status_history, options, req.body).then(function() {
                    res.redirect(redirect);
                });
            }).catch(function(err) {
                entity_helper.error(err, req, res, '/server_status_history/create_form');
            });
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/server_status_history/create_form');
...
router.get('/update_form', block_access.actionAccessMiddleware("server_status_history", "update"), function(req, res) {
    var id_e_server_status_history = req.query.id;
    var data = {
        enum_radio: enums_radios.translated("e_server_status_history", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_server_status_history', id_e_server_status_history, options).then(function(e_server_status_history) {
        ...
        e_server_status_history.dataValues.enum_radio = data.enum_radio;
        data.e_server_status_history = e_server_status_history;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_server_status_history, "e_server_status_history", true).then(function() {
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(req.query.ajax ? e_server_status_history.dataValues : data, options).then(function(data) {
                if (req.query.ajax) {
                    e_server_status_history.dataValues = data;
                    res.render('e_server_status_history/update_fields', e_server_status_history.get({
                        plain: true
                    }));
                } else
                    res.render('e_server_status_history/update', data);
            }).catch(function(err) {
                entity_helper.error(err, req, res, "/");
            })
        }).catch(function(err) {
...
router.post('/update', block_access.actionAccessMiddleware("server_status_history", "update"), function(req, res) {
    var id_e_server_status_history = parseInt(req.body.id);
    ...
    models.E_server_status_history.findOne({
        ...
        component_helper.updateAddressIfComponentExist(e_server_status_history, options, req.body);
        e_server_status_history.update(updateObject).then(function() {

            // We have to find value in req.body that are linked to an hasMany or belongsToMany association
            // because those values are not updated for now
            model_builder.setAssocationManyValues(e_server_status_history, req.body, updateObject, options).then(function() {

                var redirect = '/server_status_history/show?id=' + id_e_server_status_history;
                if (typeof req.body.associationFlag !== 'undefined')
                    redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
                ...
                entity_helper.error(err, req, res, '/server_status_history/update_form?id=' + id_e_server_status_history);
            });
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/server_status_history/update_form?id=' + id_e_server_status_history);
        });
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('server_status_history', 'read'), function(req, res) {
    ...
    // Fetch tab data
    models.E_server_status_history.findOne(queryOpts).then(function(e_server_status_history) {
        if (!e_server_status_history)
            return res.status(404).end();

        var dustData = e_server_status_history[option.as] || null;
...
router.get('/set_status/:id_server_status_history/:status/:id_new_status', block_access.actionAccessMiddleware("server_status_history", "read"), function(req, res) {
    status_helper.setStatus('e_server_status_history', req.params.id_server_status_history, req.params.status, req.params.id_new_status, req.query.comment).then(()=> {
        res.redirect(req.headers.referer);
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/server_status_history/show?id=' + req.params.id_server_status_history);
    });
});

router.post('/search', block_access.actionAccessMiddleware('server_status_history', 'read'), function(req, res) {
    ...
    models.E_server_status_history.findAndCountAll(where).then(function(results) {
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("server_status_history", "delete"), function(req, res) {
    var alias = req.params.alias;
    ...
    models.E_server_status_history.findOne({
        ...
        e_server_status_history['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(function(aliasEntities) {
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("server_status_history", "create"), function(req, res) {
    var alias = req.params.alias;
    var idEntity = req.body.idEntity;
    models.E_server_status_history.findOne({
        where: {
        ...
        if (typeof(toAdd = req.body.ids) === 'undefined') {
            ...
            return res.redirect('/server_status_history/show?id=' + idEntity + "#" + alias);
        }

        e_server_status_history['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(function() {
            res.redirect('/server_status_history/show?id=' + idEntity + "#" + alias);
        }).catch(function(err) {
            entity_helper.error(err, req, res, "/");
        });
    }).catch(function(err) {
...
router.post('/delete', block_access.actionAccessMiddleware("server_status_history", "delete"), function(req, res) {
    var id_e_server_status_history = parseInt(req.body.id);
    ...
    models.E_server_status_history.findOne({
        ...
        models.E_server_status_history.destroy({
            where: {
                id: id_e_server_status_history
            }
        }).then(function() {
            ...
            var redirect = '/server_status_history/list';
            if (typeof req.body.associationFlag !== 'undefined')
                redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
            res.redirect(redirect);
            entity_helper.remove_files("e_server_status_history", deleteObject, attributes);
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/server_status_history/list');
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/server_status_history/list');
    });
