...
var attributes = require('../models/attributes/e_partitions');
var options = require('../models/options/e_partitions');
...
router.get('/list', block_access.actionAccessMiddleware("partitions", "read"), function(req, res) {
    res.render('e_partitions/list');
});

router.post('/datalist', block_access.actionAccessMiddleware("partitions", "read"), function(req, res) {
    /* Looking for include to get all associated related to data for the datalist ajax loading */
    var include = model_builder.getDatalistInclude(models, options, req.body.columns);
    filterDataTable("E_partitions", req.body, include).then(function(rawData) {
        entity_helper.prepareDatalistResult('e_partitions', rawData, req.session.lang_user).then(function(preparedData) {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("partitions", "read"), function(req, res) {
    ...
    models.E_partitions.findOne({
        where: {
            id: parseInt(sourceId)
        },
        include: include
    }).then(function(e_partitions) {
        if (!e_partitions['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]) {
            console.error('/subdatalist: count' + entity_helper.capitalizeFirstLetter(subentityAlias) + ' is undefined');
            return res.status(500).end();
        }

        e_partitions['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function(count) {
            var rawData = {
                recordsTotal: count,
                recordsFiltered: count,
                data: []
            };
            for (var i = 0; i < e_partitions[subentityAlias].length; i++)
                rawData.data.push(e_partitions[subentityAlias][i].get({
                    plain: true
                }));
...
router.get('/show', block_access.actionAccessMiddleware("partitions", "read"), function(req, res) {
    var id_e_partitions = req.query.id;
    var tab = req.query.tab;
    var data = {
        tab: tab,
        enum_radio: enums_radios.translated("e_partitions", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_partitions', id_e_partitions, options).then(function(e_partitions) {
        if (!e_partitions) {
            data.error = 404;
        ...
        /* Update local e_partitions data before show */
        data.e_partitions = e_partitions;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_partitions, "e_partitions").then(function() {
            status_helper.translate(e_partitions, attributes, req.session.lang_user);
            data.componentAddressConfig = component_helper.getMapsConfigIfComponentAddressExist("e_partitions");
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(data, options).then(function(data) {
                res.render('e_partitions/show', data);
            }).catch(function(err) {
...
router.get('/create_form', block_access.actionAccessMiddleware("partitions", "create"), function(req, res) {
    var data = {
        enum_radio: enums_radios.translated("e_partitions", req.session.lang_user, options)
    };
    ...
    // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
    entity_helper.getLoadOnStartData(data, options).then(function(data) {
        var view = req.query.ajax ? 'e_partitions/create_fields' : 'e_partitions/create';
...
router.post('/create', block_access.actionAccessMiddleware("partitions", "create"), function(req, res) {

    var createObject = model_builder.buildForRoute(attributes, options, req.body);

    models.E_partitions.create(createObject).then(function(e_partitions) {
        var redirect = '/partitions/show?id=' + e_partitions.id;
        req.session.toastr = [{
            ...
            promises.push(new Promise(function(resolve, reject) {
                models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({
                    ...
                    if (typeof association['add' + modelName] !== 'undefined') {
                        association['add' + modelName](e_partitions.id).then(resolve).catch(function(err) {
                            reject(err);
                        });
                    } else {
                        var obj = {};
                        obj[req.body.associationForeignKey] = e_partitions.id;
        ...
        model_builder.setAssocationManyValues(e_partitions, req.body, createObject, options).then(function() {
            Promise.all(promises).then(function() {
                component_helper.setAddressIfComponentExist(e_partitions, options, req.body).then(function() {
                    res.redirect(redirect);
                });
            }).catch(function(err) {
                entity_helper.error(err, req, res, '/partitions/create_form');
            });
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/partitions/create_form');
...
router.get('/update_form', block_access.actionAccessMiddleware("partitions", "update"), function(req, res) {
    var id_e_partitions = req.query.id;
    var data = {
        enum_radio: enums_radios.translated("e_partitions", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_partitions', id_e_partitions, options).then(function(e_partitions) {
        ...
        e_partitions.dataValues.enum_radio = data.enum_radio;
        data.e_partitions = e_partitions;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_partitions, "e_partitions", true).then(function() {
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(req.query.ajax ? e_partitions.dataValues : data, options).then(function(data) {
                if (req.query.ajax) {
                    e_partitions.dataValues = data;
                    res.render('e_partitions/update_fields', e_partitions.get({
                        plain: true
                    }));
                } else
                    res.render('e_partitions/update', data);
            }).catch(function(err) {
                entity_helper.error(err, req, res, "/");
            })
        }).catch(function(err) {
...
router.post('/update', block_access.actionAccessMiddleware("partitions", "update"), function(req, res) {
    var id_e_partitions = parseInt(req.body.id);
    ...
    models.E_partitions.findOne({
        ...
        component_helper.updateAddressIfComponentExist(e_partitions, options, req.body);
        e_partitions.update(updateObject).then(function() {

            // We have to find value in req.body that are linked to an hasMany or belongsToMany association
            // because those values are not updated for now
            model_builder.setAssocationManyValues(e_partitions, req.body, updateObject, options).then(function() {

                var redirect = '/partitions/show?id=' + id_e_partitions;
                if (typeof req.body.associationFlag !== 'undefined')
                    redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
                ...
                entity_helper.error(err, req, res, '/partitions/update_form?id=' + id_e_partitions);
            });
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/partitions/update_form?id=' + id_e_partitions);
        });
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('partitions', 'read'), function(req, res) {
    ...
    // Fetch tab data
    models.E_partitions.findOne(queryOpts).then(function(e_partitions) {
        if (!e_partitions)
            return res.status(404).end();

        var dustData = e_partitions[option.as] || null;
...
router.get('/set_status/:id_partitions/:status/:id_new_status', block_access.actionAccessMiddleware("partitions", "read"), function(req, res) {
    status_helper.setStatus('e_partitions', req.params.id_partitions, req.params.status, req.params.id_new_status, req.query.comment).then(()=> {
        res.redirect(req.headers.referer);
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/partitions/show?id=' + req.params.id_partitions);
    });
});

router.post('/search', block_access.actionAccessMiddleware('partitions', 'read'), function(req, res) {
    ...
    models.E_partitions.findAndCountAll(where).then(function(results) {
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("partitions", "delete"), function(req, res) {
    var alias = req.params.alias;
    ...
    models.E_partitions.findOne({
        ...
        e_partitions['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(function(aliasEntities) {
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("partitions", "create"), function(req, res) {
    var alias = req.params.alias;
    var idEntity = req.body.idEntity;
    models.E_partitions.findOne({
        where: {
        ...
        if (typeof(toAdd = req.body.ids) === 'undefined') {
            ...
            return res.redirect('/partitions/show?id=' + idEntity + "#" + alias);
        }

        e_partitions['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(function() {
            res.redirect('/partitions/show?id=' + idEntity + "#" + alias);
        }).catch(function(err) {
            entity_helper.error(err, req, res, "/");
        });
    }).catch(function(err) {
...
router.post('/delete', block_access.actionAccessMiddleware("partitions", "delete"), function(req, res) {
    var id_e_partitions = parseInt(req.body.id);
    ...
    models.E_partitions.findOne({
        ...
        models.E_partitions.destroy({
            ...
            var redirect = '/partitions/list';
            if (typeof req.body.associationFlag !== 'undefined')
                redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
            res.redirect(redirect);
            entity_helper.remove_files("e_partitions", deleteObject, attributes);
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/partitions/list');
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/partitions/list');
    });
