...
var attributes = require('../models/attributes/e_server_alert');
var options = require('../models/options/e_server_alert');
...
router.get('/list', block_access.actionAccessMiddleware("server_alert", "read"), function(req, res) {
    res.render('e_server_alert/list');
});

router.post('/datalist', block_access.actionAccessMiddleware("server_alert", "read"), function(req, res) {
    /* Looking for include to get all associated related to data for the datalist ajax loading */
    var include = model_builder.getDatalistInclude(models, options, req.body.columns);
    filterDataTable("E_server_alert", req.body, include).then(function(rawData) {
        entity_helper.prepareDatalistResult('e_server_alert', rawData, req.session.lang_user).then(function(preparedData) {
...
router.post('/subdatalist', block_access.actionAccessMiddleware("server_alert", "read"), function(req, res) {
    ...
    models.E_server_alert.findOne({
        ...
        if (!e_server_alert['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]) {
            console.error('/subdatalist: count' + entity_helper.capitalizeFirstLetter(subentityAlias) + ' is undefined');
            return res.status(500).end();
        }

        e_server_alert['count' + entity_helper.capitalizeFirstLetter(subentityAlias)]().then(function(count) {
            var rawData = {
                recordsTotal: count,
                recordsFiltered: count,
                data: []
            };
            for (var i = 0; i < e_server_alert[subentityAlias].length; i++)
                rawData.data.push(e_server_alert[subentityAlias][i].get({
                    plain: true
                }));
...
router.get('/show', block_access.actionAccessMiddleware("server_alert", "read"), function(req, res) {
    var id_e_server_alert = req.query.id;
    var tab = req.query.tab;
    var data = {
        tab: tab,
        enum_radio: enums_radios.translated("e_server_alert", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_server_alert', id_e_server_alert, options).then(function(e_server_alert) {
        if (!e_server_alert) {
            data.error = 404;
        ...
        /* Update local e_server_alert data before show */
        data.e_server_alert = e_server_alert;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_server_alert, "e_server_alert").then(function() {
            status_helper.translate(e_server_alert, attributes, req.session.lang_user);
            data.componentAddressConfig = component_helper.getMapsConfigIfComponentAddressExist("e_server_alert");
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(data, options).then(function(data) {
                res.render('e_server_alert/show', data);
            }).catch(function(err) {
...
router.get('/create_form', block_access.actionAccessMiddleware("server_alert", "create"), function(req, res) {
    var data = {
        enum_radio: enums_radios.translated("e_server_alert", req.session.lang_user, options)
    };
    ...
    // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
    entity_helper.getLoadOnStartData(data, options).then(function(data) {
        var view = req.query.ajax ? 'e_server_alert/create_fields' : 'e_server_alert/create';
...
router.post('/create', block_access.actionAccessMiddleware("server_alert", "create"), function(req, res) {

    var createObject = model_builder.buildForRoute(attributes, options, req.body);

    models.E_server_alert.create(createObject).then(function(e_server_alert) {
        var redirect = '/server_alert/show?id=' + e_server_alert.id;
        req.session.toastr = [{
            ...
            promises.push(new Promise(function(resolve, reject) {
                models[entity_helper.capitalizeFirstLetter(req.body.associationSource)].findOne({
                    ...
                    if (typeof association['add' + modelName] !== 'undefined') {
                        association['add' + modelName](e_server_alert.id).then(resolve).catch(function(err) {
                            reject(err);
                        });
                    } else {
                        var obj = {};
                        obj[req.body.associationForeignKey] = e_server_alert.id;
        ...
        model_builder.setAssocationManyValues(e_server_alert, req.body, createObject, options).then(function() {
            Promise.all(promises).then(function() {
                component_helper.setAddressIfComponentExist(e_server_alert, options, req.body).then(function() {
                    res.redirect(redirect);
                });
            }).catch(function(err) {
                entity_helper.error(err, req, res, '/server_alert/create_form');
            });
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/server_alert/create_form');
...
router.get('/update_form', block_access.actionAccessMiddleware("server_alert", "update"), function(req, res) {
    var id_e_server_alert = req.query.id;
    var data = {
        enum_radio: enums_radios.translated("e_server_alert", req.session.lang_user, options)
    };
    ...
    entity_helper.optimizedFindOne('E_server_alert', id_e_server_alert, options).then(function(e_server_alert) {
        ...
        e_server_alert.dataValues.enum_radio = data.enum_radio;
        data.e_server_alert = e_server_alert;
        // Update some data before show, e.g get picture binary
        entity_helper.getPicturesBuffers(e_server_alert, "e_server_alert", true).then(function() {
            // Get association data that needed to be load directly here (to do so set loadOnStart param to true in options).
            entity_helper.getLoadOnStartData(req.query.ajax ? e_server_alert.dataValues : data, options).then(function(data) {
                if (req.query.ajax) {
                    e_server_alert.dataValues = data;
                    res.render('e_server_alert/update_fields', e_server_alert.get({
                        plain: true
                    }));
                } else
                    res.render('e_server_alert/update', data);
            }).catch(function(err) {
                entity_helper.error(err, req, res, "/");
            })
        }).catch(function(err) {
...
router.post('/update', block_access.actionAccessMiddleware("server_alert", "update"), function(req, res) {
    var id_e_server_alert = parseInt(req.body.id);
    ...
    models.E_server_alert.findOne({
        ...
        component_helper.updateAddressIfComponentExist(e_server_alert, options, req.body);
        e_server_alert.update(updateObject).then(function() {

            // We have to find value in req.body that are linked to an hasMany or belongsToMany association
            // because those values are not updated for now
            model_builder.setAssocationManyValues(e_server_alert, req.body, updateObject, options).then(function() {

                var redirect = '/server_alert/show?id=' + id_e_server_alert;
                if (typeof req.body.associationFlag !== 'undefined')
                    redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
                ...
                entity_helper.error(err, req, res, '/server_alert/update_form?id=' + id_e_server_alert);
            });
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/server_alert/update_form?id=' + id_e_server_alert);
        });
...
router.get('/loadtab/:id/:alias', block_access.actionAccessMiddleware('server_alert', 'read'), function(req, res) {
    ...
    // Fetch tab data
    models.E_server_alert.findOne(queryOpts).then(function(e_server_alert) {
        if (!e_server_alert)
            return res.status(404).end();

        var dustData = e_server_alert[option.as] || null;
...
router.get('/set_status/:id_server_alert/:status/:id_new_status', block_access.actionAccessMiddleware("server_alert", "read"), function(req, res) {
    status_helper.setStatus('e_server_alert', req.params.id_server_alert, req.params.status, req.params.id_new_status, req.query.comment).then(()=> {
        res.redirect(req.headers.referer);
    }).catch((err)=> {
        entity_helper.error(err, req, res, '/server_alert/show?id=' + req.params.id_server_alert);
    });
});

router.post('/search', block_access.actionAccessMiddleware('server_alert', 'read'), function(req, res) {
    ...
    models.E_server_alert.findAndCountAll(where).then(function(results) {
...
router.post('/fieldset/:alias/remove', block_access.actionAccessMiddleware("server_alert", "delete"), function(req, res) {
    var alias = req.params.alias;
    ...
    models.E_server_alert.findOne({
        ...
        e_server_alert['remove' + entity_helper.capitalizeFirstLetter(alias)](idToRemove).then(function(aliasEntities) {
...
router.post('/fieldset/:alias/add', block_access.actionAccessMiddleware("server_alert", "create"), function(req, res) {
    var alias = req.params.alias;
    var idEntity = req.body.idEntity;
    models.E_server_alert.findOne({
        where: {
        ...
        if (typeof(toAdd = req.body.ids) === 'undefined') {
            ...
            return res.redirect('/server_alert/show?id=' + idEntity + "#" + alias);
        }

        e_server_alert['add' + entity_helper.capitalizeFirstLetter(alias)](toAdd).then(function() {
            res.redirect('/server_alert/show?id=' + idEntity + "#" + alias);
        }).catch(function(err) {
            entity_helper.error(err, req, res, "/");
        });
    }).catch(function(err) {
...
router.post('/delete', block_access.actionAccessMiddleware("server_alert", "delete"), function(req, res) {
    var id_e_server_alert = parseInt(req.body.id);
    ...
    models.E_server_alert.findOne({
        ...
        models.E_server_alert.destroy({
            ...
            var redirect = '/server_alert/list';
            if (typeof req.body.associationFlag !== 'undefined')
                redirect = '/' + req.body.associationUrl + '/show?id=' + req.body.associationFlag + '#' + req.body.associationAlias;
            res.redirect(redirect);
            entity_helper.remove_files("e_server_alert", deleteObject, attributes);
        }).catch(function(err) {
            entity_helper.error(err, req, res, '/server_alert/list');
        });
    }).catch(function(err) {
        entity_helper.error(err, req, res, '/server_alert/list');
    });
